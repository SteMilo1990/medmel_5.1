...
function updateStaves(changingStyle = false) {
    ...
    if (loadedIdStaves != null && loadedIdStaves != "")
        window.history.replaceState(null, null, "?id="+loadedIdStaves);
    else
    ...
    // IF we need to keep Modern and Old distint
    if (changingStyle){
      // when switching, set the visible text area to the saved version (modern or old)
      if (currentStyle == 0) {
        notes = modernStaves;
        if (!syncModernOld) selectedText = 0;
        
      } else { // currentStyle == 1
        notes = oldStaves;
        if (!syncModernOld) selectedText = 1;
      }
      document.getElementById("text_input").value = texts[selectedText];
    }else{
      // when NOT switching (at every edit), update the current version (modern or old) to what is in the textarea
      if (currentStyle == 0) {
        modernStaves = document.getElementById("music_input").value;
        notes = modernStaves;
        if (!syncModernOld){
          texts[0] = document.getElementById("text_input").value;
        }
      } else {
        ...
        if (!syncModernOld){
          texts[1] = document.getElementById("text_input").value;
        }
      }
    }
    ...
    notes = notes.replace("##", "#");
    notes = notes.replace("%%", "%");
    notes = notes.replace("__", "_");
    notes = notes.replace("]]", "]");
    notes = notes.replace("[[", "[");
    notes = notes.replace("((", "(");
    notes = notes.replace("))", ")");
    notes = notes.replace("///", "//");
    notes = notes.replace(/f1/g, 'F1');
    notes = notes.replace(/f2/g, 'F2');
    notes = notes.replace(/f3/g, 'F3');
    notes = notes.replace(/f4/g, 'F4');
    notes = notes.replace(/f5/g, 'F5');
    notes = notes.replace(/c1/g, 'C1');
    notes = notes.replace(/c2/g, 'C2');
    notes = notes.replace(/c3/g, 'C3');
    notes = notes.replace(/c4/g, 'C4');
    notes = notes.replace(/' /g, '\'');

    // Clean text and annotations
    document.getElementById('text_input').value = document.getElementById('text_input').value.replace("\"","");
    document.getElementById('annotationsBox').value = document.getElementById('annotationsBox').value.replace("\"","");
    
    if (syncModernOld){
      if (oldStaves  == "") modernStaves = notes;
      if (modernStaves == "") oldStaves = notes;
    }
    document.getElementById("music_input").value = notes;

    let title = document.getElementById("title_input").value;
    let id_input = document.getElementById("id_input").value;
    ...
    let callingFrom = localStorage.getItem("current_page");
    ...
    try {
        texts[selectedText] = document.getElementById('text_input').value;
        annotations[selectedAnnotations] = document.getElementById('annotationsBox').value;
     } catch {}

    // Check style classic or modern
    let select = document.getElementById("shapeSelection").value;
    currentStyle = select;
    
    // Modern stemless notation
    if (currentStyle == 0) {
        if (notes != modernStaves && syncModernOld) {
            checkLineDeleted(notes, modernStaves);
            checkNoteDeleted(notes, modernStaves);
            setTimeout(function() {
                modernStaves = notes;
            }, 100);
        }
        notes = cleanKeys(notes, false);
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                resizeStaves();
                if (notes != "") highlightSelectedNote();
            }
        });
    }
    ...
    // Medieval notations
        ...
        //!!!!!!  THIS never passes because oldStaves is already synced at this point :( NEEDS FIXING
        if ((notes != oldStaves && syncModernOld) || (notes != oldStaves && !changingStyle)) {
            checkLineDeleted(notes, oldStaves);
            checkNoteDeleted(notes, oldStaves);
        }

        setTimeout(function() {
            if (syncDatasetShapes) {
              oldStaves = notes;
              syncDatasetShapes();
            ...
            // Square notation
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        if (notes != "" && page != "viewer.html") highlightSelectedNote();
                    }
                });
            }
        }, 100);
...
function resizeStaves() {
    ...
    try{
        if (page == "viewer.html") {
            let largestLineWidth = document.getElementById('largest_line').value;
            let width = document.getElementById('stavesBox').offsetWidth;
            let diff = width - largestLineWidth;
            //* This de-centers the staves etc, so I have commented it out.
            //* Why did I introduce it? It seemed to be a sloppy attempt at solving a problem linked to the resize
            // if (currentStyle == 0)
            //     diff = (diff-150)/2;
            // else {
            //     diff = (diff-75)/2;
            ...
            // //console.log(diff);
            // if (diff > 0 ) {
            //     document.getElementById("staves").style.marginLeft = ""+diff+"px";
            //     document.getElementById("id").style.marginLeft = ""+diff+"px";
            //     document.getElementById("ms").style.marginLeft = ""+diff+"px";
            // }
            // else {
            //     document.getElementById("staves").style.marginLeft = "0px";
            //     document.getElementById("id").style.marginLeft = "0px";
            //     document.getElementById("ms").style.marginLeft = "0px";
            // }
        }
    }catch(err){console.log(err);}
...
function statusHideOldInput() {
    if (document.getElementById('checkOldVisible').checked) {
        document.cookie = "hideOldInput=1";
        document.getElementById("music_input").disabled = true;
        document.getElementById("shapeSelection").disabled = true;
        modernStaves = document.getElementById("music_input").value;

        let hideText = cleanKeys(modernStaves, false);
        hideText = hideText.replace(/  +/g, ' ');
        document.getElementById("music_input").value = hideText;
    }
    else{
        document.cookie = "hideOldInput=";
        document.getElementById("music_input").disabled = false;
...
function setStavesStyle() {
    const currentPage = localStorage.getItem("current_page");
    currentStyle = document.getElementById("shapeSelection").value;
    ...
    if (syncModernOld && currentPage !== "viewer.html") {
      if (currentStyle == 0) {
          ...
          document.getElementById("music_input").value = modernStaves;
          try {
            document.getElementById("btnBarMode").style.display = "none";
            document.getElementById("btnCustosMode").style.display = "none";
            document.getElementById("btnDefaultShapes").style.display = "none";
          } catch {}
      }
      else {
          try {
            document.getElementById("btnBarMode").style.display = "block";
            document.getElementById("btnCustosMode").style.display = "block";
            document.getElementById("btnDefaultShapes").style.display = "block";
          } catch {}
...
function extractGroupFromClick(nGroup, line) {
    ...
    // else if (groupNote.length == 5 && notationType == 0) {
    //   $(containerNoteMenu).load("./Menu/menuGroup5Square.html");
    //   setTimeout(function() {
    //       setupButtons5(indexGroup, groupNote, groupLine, groupPos);
    //   }, 150);
    //   setTimeout(function() {
...
function syncDatasetShapes() {
    ...
    for(let i = 0; i < notes.length; i = i+1) {
        try{
            ...
            if (typeof shapeGroupNote[i] == 'undefined') shapeGroupNote[i] = [];
            //setup shapeSingleNote
            if (typeof shapeSingleNote[i] == 'undefined') shapeSingleNote[i] = [];
            //setup stemSingleNote
            if (typeof stemSingleNote[i] == 'undefined') stemSingleNote[i] = [];
            //setup shapeGroupNote
            if (typeof connectGroupNote[i] == 'undefined') connectGroupNote[i] = [];
            //setup bar array
            if (typeof bar[i] == 'undefined') bar[i] = [];
            if (typeof bar[i][0] == 'undefined') bar[i][0] = [];
            if (typeof bar[i][1] == 'undefined') bar[i][1] = [];
            groupSplit = notes[i].split(" ");

            for (let j = 0; j < groupSplit.length; j = j+1) { //for every group in line

                //setup shapeSingleNote
                if (typeof shapeSingleNote[i][j] == 'undefined') shapeSingleNote[i][j] = [];
                //setup stemSingleNote
                if (typeof stemSingleNote[i][j] === 'undefined') stemSingleNote[i][j] = []; 
                //setup shapeGroupNote
                if (typeof connectGroupNote[i][j] == 'undefined') connectGroupNote[i][j] = [];
                //setup bar
                if (typeof bar[i][j] == 'undefined') bar[i][j] = [];

                for(let x = 0; x < groupSplit[j].length; x = x+1) {
                    //setup shapeGroupNote
                    if (x+1 < groupSplit[j].length)
                        if (typeof connectGroupNote[i][j][x+1] == 'undefined') connectGroupNote[i][j][x+1] = 1;

                    if (groupSplit[j].length > 1 && groupSplit[j].length < 4 ) {}
                    else {
                        // //setup shapeSingleNote
                        // if (typeof shapeSingleNote[i][j][x] == 'undefined') shapeSingleNote[i][j][x] = 1;
                        // //setup stemSingleNote
                        // if (typeof stemSingleNote[i][j][x] == 'undefined') stemSingleNote[i][j][x] = 1;
                    }
...
//---------------print dataset to console----------------------
    // str = "";
    // str = "shapeGroupNote : \n";
    // for(let i = 0; i < shapeGroupNote.length; i = i+1) {//line
    //     str = str+ ""+i+" : [";
    //     //groupSplit = notes[i].split(" ");
    //     try{
    //         for (let j = 0; j < shapeGroupNote[i].length; j = j+1) {
    //             if (j != 0) str =  str +", ";
    //             str = str +shapeGroupNote[i][j];
    //
    //         }
    //     }catch(err){console.log(err);}
    //
    ...
    // str = "";
    // str = "shapeSingleNote : \n";
    // for(let i = 0; i < shapeSingleNote.length; i = i+1) {//line
    //     str = str+ ""+i+" : [";
    //     //groupSplit = notes[i].split(" ");
    //     try{
    //         for (let j = 0; j < shapeSingleNote[i].length; j = j+1) {
    //             str = str +"["+shapeSingleNote[i][j]+"]";
    //
    //         }
    //     }catch(err){console.log(err);}
    //
    ...
    // str = "";
    // str = "stemSingleNote : \n";
    // for(let i = 0; i < stemSingleNote.length; i = i+1) {//line
    //     str = str+ ""+i+" : [";
    //     //groupSplit = notes[i].split(" ");
    //     try{
    //         for (let j = 0; j < stemSingleNote[i].length; j = j+1) {
    //             str = str +"["+stemSingleNote[i][j]+"]";
    //
    //         }
    //     }catch(err){console.log(err);}
    //
    ...
    // //console.log(str);
    // str = "";
    // str = "connectGroupNote : \n";
    // for(let i = 0; i < connectGroupNote.length; i = i+1) {//line
    //     str = str+ ""+i+" : [";
    //     //groupSplit = notes[i].split(" ");
    //     try{
    //         for (let j = 0; j < connectGroupNote[i].length; j = j+1) {
    //             str = str +"["+connectGroupNote[i][j]+"]";
    //
    //         }
    //     }catch(err){console.log(err);}
    //
