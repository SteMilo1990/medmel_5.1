...
function playMusic(){
    ...
    let pattern = getDurationPattern(formatNotes(text, true));
...
function formatNotes(str, forDurationPatter=false){
...
function getDurationPattern(notes){
    ...
    pattern = pattern.replace(/J|A|B|H|C|D|E|F|G|a|b|h|c|d|e|f|g|u|p|q|r|s/g, '0');
...
function resetUIbuttonPause() {
    document.getElementById("btnPlay").title = "Pause";
    let img = document.getElementById("playImg");
    let imgOverlay = document.getElementById("playImgOverlay");
    img.src ="img/icons/pause1.png";
    imgOverlay.src ="img/icons/pause1.png";
}
function resetUIbuttonPlay() {
    document.getElementById("btnPlay").title = "Play";
    let img = document.getElementById("playImg");
    let imgOverlay = document.getElementById("playImgOverlay");
    img.src ="img/icons/play1.png";
    imgOverlay.src ="img/icons/play1.png";
}
function clickStopPlayer() {
    resetUIbuttonPlay();
    stopMusic();

}
function btnStopPlayer() {
    resetUIbuttonPlay();
    let textarea = document.getElementById("music_input");
    textarea.selectionEnd = 0;
    textarea.focus();
...
function btnNext() {
    ...
    let strNotes = textarea.value;
    let delta = 0;
    let row = 0;
    while((cursor+delta) < strNotes.length && strNotes.charAt(cursor+delta) != '\n') {
        delta = delta + 1;
    }
    if(strNotes.charAt(cursor+delta) == '\n' ) {
        textarea.selectionEnd = (cursor+delta+1);
        textarea.selectionStart = (cursor+delta+1);
        textarea.focus();
        updateStaves();
    }
    else {
...
function btnPrev() {
    ...
    if((cursor+delta)>=0 ) {
        ...
        if(cursor+delta == 0) {
            ...
            textarea.selectionEnd = cursor+delta+2;
            textarea.selectionStart = cursor+delta+2;
        }
...
function changeSpeed() {
    ...
    label.innerHTML = "Music Speed: "+speed+"x";

    document.getElementById("speedOverlay").value = speed;
    let labelOverlay = document.getElementById("txtSpeedOverlay");
    labelOverlay.innerHTML = "Music Speed: "+speed+"x";
...
function changeOctave() {
    ...
    label.innerHTML = "Octave: "+octave+"";

    document.getElementById("octaveOverlay").value = octave;
    let labelOverlay = document.getElementById("txtOctaveOverlay");
    labelOverlay.innerHTML = "Octave: "+octave+"";
